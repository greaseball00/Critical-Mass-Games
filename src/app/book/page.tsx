    ...
    return (
      <div style={{ maxWidth: '500px', margin: '6rem auto', padding: '0 1.5rem', textAlign: 'center' }}>
        ...
        <div className="panel glow-border" style={{ padding: '2.5rem' }}>
          <LogIn size={36} style={{ color: '#39ff14', margin: '0 auto 1rem' }} />
          <h2 style={{ color: '#39ff14', fontWeight: 700, fontSize: '1.5rem', letterSpacing: '0.05em', marginBottom: '0.75rem' }}>
            Log In to Book
          </h2>
          <p style={{ color: '#7ab87a', lineHeight: 1.7, marginBottom: '2rem' }}>
      ...
      <div style={{ maxWidth: '600px', margin: '6rem auto', padding: '0 1.5rem', textAlign: 'center' }}>
        <div className="panel glow-border" style={{ padding: '3rem' }}>
          ...
          <p style={{ color: '#7ab87a', lineHeight: 1.7, marginBottom: '1.5rem' }}>
            Your booking for <strong style={{ color: '#c8f5c2' }}>{tableData?.name}</strong> on{' '}
            <strong style={{ color: '#c8f5c2' }}>{form.date}</strong> at{' '}
            <strong style={{ color: '#c8f5c2' }}>{form.timeSlot}</strong> is confirmed.
          </p>
          <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '3rem 1.5rem' }}>
        ...
        {['Select Table', 'Session Details', 'Confirm'].map((label, i) => {
          ...
          return (
            <div key={label} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.4rem', position: 'relative' }}>
              <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: done ? '#39ff14' : active ? 'rgba(57,255,20,0.15)' : 'rgba(57,255,20,0.05)', border: `2px solid ${done || active ? '#39ff14' : 'rgba(57,255,20,0.2)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: done ? '#050a05' : active ? '#39ff14' : '#4a6b4a', fontSize: '0.8rem', fontWeight: 700 }}>
              ...
              {i < 2 && <div style={{ position: 'absolute', top: '16px', left: '50%', width: '100%', height: '2px', background: done ? '#39ff14' : 'rgba(57,255,20,0.15)', zIndex: -1 }} />}
            </div>
          );
        })}
      </div>
      ...
      {step === 1 && (
        <>
            ...
            {tables.map(table => {
              ...
              return (
                <div
                  ...
                  className="panel"
                  style={{ padding: '1.5rem', cursor: isUnavailable ? 'not-allowed' : 'pointer', opacity: isUnavailable ? 0.5 : 1, border: isSelected ? '2px solid #39ff14' : (table as { flagship?: boolean }).flagship ? '1px solid rgba(57,255,20,0.4)' : '1px solid rgba(57,255,20,0.2)', boxShadow: isSelected ? '0 0 20px rgba(57,255,20,0.4)' : 'none', transition: 'all 0.2s', position: 'relative' }}
                >
                  {(table as { flagship?: boolean }).flagship && (
                    <div style={{ position: 'absolute', top: 0, right: 0, background: '#39ff14', color: '#050a05', padding: '0.15rem 0.5rem', fontSize: '0.6rem', fontWeight: 700, letterSpacing: '0.1em' }}>FLAGSHIP</div>
                  )}
          ...
          {/* Info strip */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginTop: '3rem', borderTop: '1px solid rgba(57,255,20,0.1)', paddingTop: '2rem' }}>
            {[
      ...
      {step === 2 && (
        <form onSubmit={e => { e.preventDefault(); setStep(3); }}>
          <div style={{ maxWidth: '600px', display: 'flex', flexDirection: 'column', gap: '1.25rem', marginBottom: '2rem' }}>
            <div style={{ background: 'rgba(57,255,20,0.05)', border: '1px solid rgba(57,255,20,0.2)', padding: '0.75rem 1rem', fontSize: '0.85rem', color: '#7ab87a' }}>
            ...
            {[
              ...
              <div key={field.name}>
                <label style={{ display: 'block', fontSize: '0.75rem', color: '#7ab87a', letterSpacing: '0.1em', marginBottom: '0.4rem', textTransform: 'uppercase' }}>{field.label}</label>
      ...
      {step === 3 && (
        <div style={{ maxWidth: '580px' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-start', background: 'rgba(57,255,20,0.05)', border: '1px solid rgba(57,255,20,0.2)', padding: '1rem', marginBottom: '1.25rem', fontSize: '0.85rem' }}>
          ...
          {saveError && (
            <div style={{ background: 'rgba(255,68,68,0.1)', border: '1px solid rgba(255,68,68,0.4)', padding: '0.75rem', color: '#ff8888', fontSize: '0.85rem', marginBottom: '1.25rem' }}>
              ⚠ {saveError}
            </div>
          )}
